...
export function cPage(p, el, isForceNew = false) {
    ...
    if (p === 'new') { 
        if (isForceNew) AppState.activeManageProjectId = null; // Error Fix: Clear project state
...
// Stubs for new missing functions
...
export async function handleMultiFileUpload(input) {
    ...
    // Har ek video/audio file ka time calculate karo
...
function renderFileList() {
    ...
    container.innerHTML = AppState.newProjectFiles.map((file, idx) => {
        ...
        // Agar duration hai, toh clock icon ke sath render karo
...
export function renderF(p) {
    ...
    // YAHAN CHANGE HAI: Placeholder ki jagah fNegotiate() call kar rahe hain
...
function fNegotiate() {
    ...
    // Background mein direct Cloud se fetch karega
    setTimeout(async () => {
        if (window.supaClient) {
            try {
                ...
                if (data && !error) {
                    ...
                    // Local DB Sync
                    const allLocal = DB.projects();
                    let updated = false;
                    projs.forEach(ip => {
                        if (!allLocal.find(x => x.id === ip.id)) {
                            allLocal.push({
                                id: ip.id, creatorId: ip.creator_id, title: ip.title, description: ip.description,
                                budget: ip.budget, contentType: ip.content_type, deadline: ip.deadline, priority: ip.priority,
                                invited_freelancers: ip.invited_freelancers, status: ip.status, createdAt: new Date(ip.created_at).getTime()
                            });
                            updated = true;
                        }
                    });
                    if (updated) DB.saveProjects(allLocal);
                ...
                console.error("Negotiate Fetch Error:", e); 
            } // Bracket yahan theek kar diya gaya hai
        }
    }, 100);
    ...
    <div id="neg-main-container">
        <div style="color:var(--text-3);font-size:.85rem; padding:20px 0;"><i class="fa fa-spinner fa-spin"></i> Loading projects from cloud...</div>
    </div>`;
}
...
export async function sendNegotiation(pid) {
    ...
    // UI ko jo format chahiye wo yeh hai
    ...
    try {
        ...
        // Chat screen pe bhej do taaki wo apna bheja hua offer dekh sake
    ...
    // Sab cards ko chupa do
    document.querySelectorAll('.neg-card-item').forEach(el => el.style.display = 'none');
    // Sirf select kiya hua card dikhao
    const selected = document.getElementById('neg-card-' + pid);
    if (selected) selected.style.display = 'block';
}
...
function fBrowse() {
    ...
    // Background mein direct Cloud (Supabase) se fetch karega
    setTimeout(async () => {
        if (window.supaClient) {
            try {
                const { data, error } = await window.supaClient.from('projects').select('*').eq('status', 'open');
                if (data && !error) {
                    ...
                    // Sirf wahi filter karo jisme Anar (Freelancer) ki ID ho
                    const invitedProjs = data.filter(p => {
                        let invited = p.invited_freelancers;
                    ...
                    // Local DB mein save karo taaki 'View Details' button kaam kare
                    const allLocal = DB.projects();
                    let updated = false;
                    invitedProjs.forEach(ip => {
                        if (!allLocal.find(x => x.id === ip.id)) {
                            allLocal.push({
                                id: ip.id, creatorId: ip.creator_id, title: ip.title, description: ip.description,
                                budget: ip.budget, contentType: ip.content_type, deadline: ip.deadline, priority: ip.priority,
                                invited_freelancers: ip.invited_freelancers, status: ip.status,
                                createdAt: new Date(ip.created_at).getTime()
                            });
                            updated = true;
                        }
                    });
                        ...
                        container.innerHTML = invitedProjs.map(p => `
                            <div class="pc" style="padding:16px 20px; border-radius:12px; display:flex; flex-wrap:wrap; gap:16px; align-items:center; justify-content:space-between; background:var(--surface); border:1px solid var(--glass-border);">
                                <div style="display:flex; align-items:center; gap:16px;">
                                    <div class="pico" style="background:var(--bg2); border:1px solid var(--glass-border);">${contentIconSvg(p.content_type || p.contentType)}</div>
                                    <div class="pinfo">
                                        <div class="ptitle" style="font-size:1.05rem;">${p.title}</div>
                                        <div class="pmeta" style="margin-top:4px;">₹${fmt(p.budget)} &middot; ${p.content_type || p.contentType}</div>
                                    </div>
                                </div>
            ...
            } // Bracket ab yahan sahi jagah band hai
        }
    }, 100);

    return `
    <div class="page-head"><h2>Browse Projects</h2><p>Invitations from creators</p></div>
    <div class="project-list" id="browse-list-container">
        <div style="color:var(--text-3);font-size:.85rem;padding:20px 0;"><i class="fa fa-spinner fa-spin"></i> Checking cloud for new invitations...</div>
    </div>`;
}
...
export async function viewProjectDetails(pid) {
    ...
    // Yahan 'window.' hata diya gaya hai
...
function fOngoing() {
    ...
    <div style="display:flex; flex-direction:column; gap:16px;">
        ${projs.length ? projs.map(p => {
            // Client ka naam nikalo
            const creator = DB.users().find(u => u.id === p.creatorId);
            const cName = creator ? creator.name.split(' ')[0] : 'Creator';
            
            // Agar edited video upload ho chuki hai to 100%, nahi to 50%
...
// ─── UPLOAD FINAL WORK LOGIC (ULTIMATE FIX) ───
export async function handleFinalUpload(input, projectId) {
    if (!input.files || !input.files.length) return;

    // 1. Sirf wahi dropdown padho jo screen par currently dikh raha hai
    ...
    // 🔥 Value ko clean karo (extra spaces hata do)
    finalProjId = String(finalProjId).trim();

    const projs = DB.projects() || [];

    // 2. 3-Step Smart Matching Algorithm
    // Step A: Pehle check karo ki kya yeh direct ID hai?
    let proj = projs.find(p => String(p.id).trim() === finalProjId);

    // Step B: Agar ID nahi hai, toh Title ka exact match dhoondho (Case insensitive)
    if (!proj) {
        proj = projs.find(p => p.title && String(p.title).trim().toLowerCase() === finalProjId.toLowerCase());
    }

    // Step C: Agar phir bhi na mile, toh partial match dhoondho (e.g. "waris " -> "waris")
    if (!proj) {
        proj = projs.find(p => p.title && String(p.title).trim().toLowerCase().includes(finalProjId.toLowerCase()));
    }

    // Agar itne ke baad bhi project nahi mila, toh screen par exact galti dikhao!
    if (!proj) {
        window.showToast(`Error: Can't find "${finalProjId}" in DB`, 'err');
        console.log("Tried to find:", finalProjId);
        console.log("Available projects:", projs);
        input.value = '';
        return;
    }
...
export function viewFreelancerPortfolio(id) {
    ...
    // Check karo ki freelancer pehle se select hai ya nahi
    ...
    // Naam mein agar koi special character (jaise D'Souza) ho toh usko safe banane ke liye
...
export function reviseOffer(pid) {
    // 1. Freelancer ko wapas Negotiate tab pe bhejo
    window.fPage('negotiate', document.querySelector('[data-page=negotiate]'));
    
    // 2. Thoda wait karo (DOM load hone tak), fir dropdown select karke card open kar do
    setTimeout(() => {
...
export async function triggerApproveAndPay(projectId) {
    ...
    try {
        ...
        // 3. Send Final Success Message to Freelancer
        ...
        // UI Refresh (Chat waise hi rahegi, button download link mein badal jayega)
        setTimeout(() => {
            const msgTab = document.querySelector('[data-page=messages]');
            if(msgTab) window.fPage('messages', msgTab);
        }, 500);
