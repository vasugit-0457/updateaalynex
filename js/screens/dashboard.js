...
function renderFileList() {
    ...
    container.innerHTML = AppState.newProjectFiles.map((file, idx) => {
        ...
        let iconSvg = '';
        if (file.type.startsWith('video/')) iconSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:18px;height:18px;"><rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="2" y1="7" x2="7" y2="7"/><line x1="2" y1="17" x2="7" y2="17"/><line x1="17" y1="17" x2="22" y2="17"/><line x1="17" y1="7" x2="22" y2="7"/></svg>`;
        else if (file.type.startsWith('audio/')) iconSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:18px;height:18px;"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>`;
        else iconSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:18px;height:18px;"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>`;
        
        const durationHtml = file.displayDuration ? ` <span style="margin:0 5px;color:rgba(0,0,0,0.15);">&bull;</span> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:10px;height:10px;margin-right:3px;vertical-align:-1px;"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>${file.displayDuration}` : '';
        return `
        <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--surface);border:1px solid var(--glass-border);border-radius:var(--radius-sm);box-shadow:var(--shadow-xs);">
            <div style="width:38px;height:38px;border-radius:8px;background:var(--bg2);display:flex;align-items:center;justify-content:center;color:var(--text-2);flex-shrink:0;">${iconSvg}</div>
            <div style="flex:1;min-width:0;">
                ...
                <div style="font-size:.7rem;color:var(--text-3);margin-top:2px;display:flex;align-items:center;">${sizeStr} ${durationHtml}</div>
            </div>
            <div style="color:var(--red);cursor:pointer;font-family:sans-serif;font-weight:bold;font-size:.9rem;padding:4px 8px;" onclick="window.removeProjectFile(${idx})">x</div>
        </div>`;
    }).join('');
}
...
// ─── BACKGROUND SYNC HELPER (INVISIBLE) ───
function invisibleProjectSync() {
    setTimeout(async () => {
        if(window.supaClient) {
            try {
                const {data} = await window.supaClient.from('projects').select('*').eq('status', 'open');
                if(data) {
                    const localDB = DB.projects();
                    let isNew = false;
                    data.forEach(dp => {
                        if(!localDB.find(x => x.id === dp.id)) {
                            localDB.push({
                    ...
                    if(isNew) {
                        ...
                        // Refresh the UI silently if the user is currently on the Browse page
                        if(document.querySelector('[data-page="browse"]')?.classList.contains('active')) {
...
// ─── FREELANCER SCREENS ───
function fBrowse() {
    invisibleProjectSync(); // This will auto-fetch without showing any loading text
...
export async function triggerApproveAndPay(projectId) {
    if (!projectId || projectId === 'undefined') {
        window.showToast('Error: Project ID missing', 'err'); return;
    }
    window.showToast('Processing Payment...', 'info');
    try {
        const projs = DB.projects();
        const proj = projs.find(p => p.id === projectId);
        if (!proj) { window.showToast('Project not found!', 'err'); return; }
